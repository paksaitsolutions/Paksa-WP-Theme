<?php
/**
 * Paksa IT Solutions — Block Pattern: Hero Split
 *
 * Two equal columns: text content left, visual/image right.
 * Suited for landing pages with product or service visuals.
 *
...
register_block_pattern(
    ...
    array(
        'title'      => __( 'Hero Split', 'paksa-it-solutions' ),
        'categories' => array( 'paksa-hero' ),
        'keywords'   => array( 'hero', 'split', 'two-column', 'image' ),
        'viewport'   => array(
            'width'  => 1280,
            'height' => 600,
        ),
        'content'    => '<!-- wp:group {"className":"pk-hero pk-hero-split","align":"wide"} -->
<div class="pk-hero pk-hero-split"><!-- wp:group {"className":"pk-hero-inner"} -->
<div class="pk-hero-inner" style="display:grid;grid-template-columns:1fr;gap:3rem;align-items:center"><!-- wp:group {"className":"pk-hero-content"} -->
...
<div class="pk-hero-visual pk-animate-on-scroll" data-anim="fade-in" data-delay="300" aria-hidden="true"><!-- wp:image {"id":0,"sizeSlug":"large","linkDestination":"media"} -->
<figure class="wp-block-image size-large"><img src="' . esc_url( get_template_directory_uri() ) . '/assets/images/hero-split-placeholder.png" alt="' . esc_attr__( 'Hero visual', 'paksa-it-solutions' ) . '"/></figure><!-- /wp:image --></div><!-- /wp:group --></div><!-- /wp:group --></div><!-- /wp:group -->',
    )
);
